...
# The encryption key must live next to the user data (%APPDATA%\ApexVault),
# NOT next to the source files. When frozen with PyInstaller, __file__ points
# into a temporary _MEIxxxxxx folder that is recreated (and emptied) on every
# launch — a key stored there would be lost, making the vault undecryptable.
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_APP_DATA = os.path.join(os.environ.get("APPDATA", os.path.expanduser("~")), "ApexVault")
os.makedirs(_APP_DATA, exist_ok=True)
_KEY_FILE = os.path.join(_APP_DATA, "key.vault")
_LEGACY_KEY_FILE = os.path.join(_ROOT, "key.vault")


def _migrate_legacy_key() -> None:
    """One-time move of key.vault from the project root to %APPDATA%,
    so existing (pre-fix) vaults keep working after the upgrade."""
    if os.path.exists(_KEY_FILE) or not os.path.exists(_LEGACY_KEY_FILE):
        return
    try:
        with open(_LEGACY_KEY_FILE, "rb") as src:
            blob = src.read()
        tmp = _KEY_FILE + ".tmp"
        with open(tmp, "wb") as dst:
            dst.write(blob)
        os.replace(tmp, _KEY_FILE)
        os.remove(_LEGACY_KEY_FILE)
    except OSError:
        pass
